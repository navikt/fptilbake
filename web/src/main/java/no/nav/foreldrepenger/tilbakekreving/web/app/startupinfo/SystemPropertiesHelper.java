...
class SystemPropertiesHelper {
    ...
    private static final Pattern SECRET_NAME_PARTS = Pattern.compile("passw?o?rd?|private|key", Pattern.CASE_INSENSITIVE);
    private static final String FILTERED_VALUE = "*****";
    private static final Pattern SECRET_VERDI_REGEX = Pattern.compile("((?:passw?o?rd?|private|key)[^=]*\\s*=\\s*)[^ ]*", Pattern.CASE_INSENSITIVE);
    ...
    static void filter(Map<String, String> map) {
        for (Map.Entry<String, String> entry : map.entrySet()) {
            ...
            } else if (entry.getValue().indexOf('=') >= 0 && containsSecretNamePart(entry.getValue())) {
    ...
    private static boolean containsSecretNamePart(String name) {
        return SECRET_NAME_PARTS.matcher(name).find();
    }
}

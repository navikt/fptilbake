...
public interface DokumentArkivTjenesteFeil extends DeklarerteFeil {
    ...
    @ManglerTilgangFeil(feilkode = "FPT-751934", feilmelding = "Mangler tilgang til å utføre '%s' mot Journalsystemet", logLevel = LogLevel.ERROR)
    Feil journalUtilgjengeligSikkerhetsbegrensning(String operasjon, Exception e);

    @IntegrasjonFeil(feilkode = "FPT-195533", feilmelding = "Journalpost ikke funnet", logLevel = LogLevel.WARN)
    Feil hentJournalpostIkkeFunnet(HentDokumentJournalpostIkkeFunnet e);

    @IntegrasjonFeil(feilkode = "FPT-276511", feilmelding = "Journalpost ugyldig input", logLevel = LogLevel.WARN)
    Feil journalpostUgyldigInput(HentKjerneJournalpostListeUgyldigInput e);

    @IntegrasjonFeil(feilkode = "FPT-249790", feilmelding = "Fant ikke journal dokument", logLevel = LogLevel.WARN)

...
public interface JournalFeil extends DeklarerteFeil {
    ...
    @ManglerTilgangFeil(feilkode = "FPT-751834", feilmelding = "Mangler tilgang til å utføre '%s' mot Journalsystemet", logLevel = LogLevel.ERROR)
    Feil journalUtilgjengeligSikkerhetsbegrensning(String operasjon, Exception e);

    @IntegrasjonFeil(feilkode = "FPT-195433", feilmelding = "Journalpost ikke funnet", logLevel = LogLevel.WARN)
    Feil hentJournalpostIkkeFunnet(HentJournalpostJournalpostIkkeFunnet e);

    @IntegrasjonFeil(feilkode = "FPT-276411", feilmelding = "Journalpost ugyldig input", logLevel = LogLevel.WARN)
    Feil journalpostUgyldigInput(HentJournalpostUgyldigInput e);

    @IntegrasjonFeil(feilkode = "FPT-107540", feilmelding = "Journalpost ikke inngående", logLevel = LogLevel.WARN)

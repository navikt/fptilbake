...
public interface TpsFeilmeldinger extends DeklarerteFeil {
    ...
    @TekniskFeil(feilkode = "FPT-164686", feilmelding = "Person er ikke Bruker, kan ikke hente ut brukerinformasjon", logLevel = WARN)
    Feil ukjentBrukerType();

    @ManglerTilgangFeil(feilkode = "FPT-432142", feilmelding = "TPS ikke tilgjengelig (sikkerhetsbegrensning)", logLevel = ERROR)
    Feil tpsUtilgjengeligSikkerhetsbegrensning(HentPersonSikkerhetsbegrensning cause);

    @TekniskFeil(feilkode = "FPT-715013", feilmelding = "Fant ikke person i TPS", logLevel = WARN)

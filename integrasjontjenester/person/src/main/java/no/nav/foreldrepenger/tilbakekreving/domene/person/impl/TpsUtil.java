...
public class TpsUtil {
    private TpsUtil() {
        // blokker instanser av util klasse
    }
    ...
    static no.nav.foreldrepenger.tilbakekreving.domene.typer.PersonIdent getPersonIdent(Bruker bruker) {
        return getPersonIdent(bruker.getAktoer());
    }

    static no.nav.foreldrepenger.tilbakekreving.domene.typer.PersonIdent getPersonIdent(Aktoer aktor) {
        if (!(aktor instanceof PersonIdent)) {
            throw new IllegalArgumentException("Krever at person inneholder en " + PersonIdent.class + " men fikk " + (aktor != null ? aktor.getClass().getName() : null));
        }
        PersonIdent pi = (PersonIdent) aktor;
        String fnr = pi.getIdent().getIdent();
        return no.nav.foreldrepenger.tilbakekreving.domene.typer.PersonIdent.fra(fnr);
    }

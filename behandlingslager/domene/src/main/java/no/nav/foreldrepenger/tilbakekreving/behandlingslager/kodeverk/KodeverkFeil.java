...
interface KodeverkFeil extends DeklarerteFeil {
    ...
    @TekniskFeil(feilkode = "FPT-314678", feilmelding = "Kan ikke finne kodeverk for type '%s', kode '%s'", logLevel = LogLevel.WARN)
    Feil kanIkkeFinneKodeverk(String klassetype, String kode, NoResultException e);

    @TekniskFeil(feilkode = "FPT-314679", feilmelding = "Kan ikke finne kodeverk for type '%s', kode '%s'", logLevel = LogLevel.WARN)
    Feil kanIkkeFinneKodeverkForOffisiellKode(String klassetype, String kode, NoResultException e);

    @TekniskFeil(feilkode = "FPT-314680", feilmelding = "Kan ikke finne kodeverk for type '%s', kode '%s'", logLevel = LogLevel.WARN)

...
interface HistorikkInnsalgFeil extends DeklarerteFeil {
    ...
    @TekniskFeil(feilkode = "FPT-876694", feilmelding = "For type %s, mangler felter %s for historikkinnslag.", logLevel = ERROR)
    Feil manglerFeltForHistorikkInnslag(String type, List<String> manglendeFelt);

    @TekniskFeil(feilkode = "FPT-876693", feilmelding = "For type %s, forventer minst et felt av type %s", logLevel = ERROR)
    Feil manglerMinstEtFeltForHistorikkinnslag(String type, List<String> manglendeFelt);

    @TekniskFeil(feilkode = "FPT-876692", feilmelding = "Ukjent historikkinnslagstype: %s", logLevel = ERROR)

...
public enum HendelseUnderType implements Kodeverdi {
    ...
    STONADSPERIODE_OVER_3("STONADSPERIODE_OVER_3", "Stønadsperioden er over 3 år", 0),
    NY_STONADSPERIODE("NY_STONADSPERIODE", "Ny stønadsperiode for nytt barn", 1),
    IKKE_OMSORG("IKKE_OMSORG", "Ikke omsorg for barnet", 2),
    MOTTAKER_I_ARBEID("MOTTAKER_I_ARBEID", "Mottaker er i arbeid heltid", 3),
    ...
    MOTTAKER_HELT_AVHENGIG("MOTTAKER_HELT_AVHENGIG", "Mottaker er helt avhengig av hjelp til å ta seg av barnet", 2),
    MOTTAKER_INNLAGT("MOTTAKER_INNLAGT", "Mottaker er innlagt i helseinstitusjon", 3),
    BARN_INNLAGT("BARN_INNLAGT", "Barn er innlagt i helseinstitusjon", 4),
    KVO_MOTTAKER_HELT_AVHENGIG("KVO_MOTTAKER_HELT_AVHENGIG", "Mottaker er helt avhengig av hjelp til å ta seg av barnet", 0),
    KVO_MOTTAKER_INNLAGT("KVO_MOTTAKER_INNLAGT", "Mottaker er innlagt i helseinstitusjon", 1),
    KVO_SAMTIDIG_UTTAK("KVO_SAMTIDIG_UTTAK", "Samtidig uttak", 3),
    MOR_IKKE_ARBEID("MOR_IKKE_ARBEID", "Mor er ikke i arbeid heltid", 0),
    MOR_IKKE_STUDERT("MOR_IKKE_STUDERT", "Mor er ikke student på heltid", 1),
    MOR_IKKE_ARBEID_OG_STUDER("MOR_IKKE_ARBEID_OG_STUDER", "Mor er ikke i arbeid og student - heltid", 2),
    MOR_IKKE_HELT_AVHENGIG("MOR_IKKE_HELT_AVHENGIG", "Mor er ikke helt avhengig av hjelp til å ta seg av barnet", 3),
    MOR_IKKE_INNLAGT("MOR_IKKE_INNLAGT", "Mor er ikke innlagt på helseinstitusjon", 4),
    MOR_IKKE_I_IP("MOR_IKKE_I_IP", "Mor er ikke i introduksjonsprogram", 5),
    MOR_IKKE_I_KP("MOR_IKKE_I_KP", "Mor er ikke i kvalifiseringsprogram", 6),
    ...
    SVP_IKKE_HELSEFARLIG("SVP_IKKE_HELSEFARLIG", "Ikke helsefarlig for ventet barn", 2),
    SVP_TILRETTELEGGING_FULLT_MULIG("SVP_TILRETTELEGGING_FULLT_MULIG", "Full tilrettelegging mulig", 0),
    SVP_TILRETTELEGGING_DELVIS_MULIG("SVP_TILRETTELEGGING_DELVIS_MULIG", "Delvis tilrettelegging mulig", 1),
    ...
    MOTTAKER_DØD("MOTTAKER_DØD", "Mottaker er død", 1),
    MOTTAKER_IKKE_GRAVID("MOTTAKER_IKKE_GRAVID", "Mottaker er ikke lenger gravid", 2),
    SVP_INNTEKT_IKKE_TAP("SVP_INNTEKT_IKKE_TAP", "Ikke tap av pensjonsgivende inntekt", 20),
    //ES
    ES_BARN_IKKE_REGISTRERT("ES_BARN_IKKE_REGISTRERT", "Barn er ikke registrert", 0),
    ES_MOTTAKER_FAR_MEDMOR("ES_MOTTAKER_FAR_MEDMOR", "Mottaker er far eller medmor", 1),
    ES_IKKE_OPPFYLT("ES_IKKE_OPPFYLT", "Adopsjonsvilkår er ikke oppfylt", 0),
    ES_BARN_OVER_15("ES_BARN_OVER_15", "Barn er over 15 år", 1),
    ES_MANN_IKKE_ALENE("ES_MANN_IKKE_ALENE", "Mann adopterer ikke alene", 2),
    ES_STEBARN("ES_STEBARN", "Stebarnsadopsjon", 3),
    ES_ANDRE_FORELDRE_DODD("ES_ANDRE_FORELDRE_DODD", "Har ikke foreldreansvar ved andre forelders død", 0),
    ...
    ES_FORELDREANSVAR_BARN_OVER_15("ES_FORELDREANSVAR_BARN_OVER_15", "Barn er over 15 år", 3),
    ES_FAR_IKKE_OMSORG("ES_FAR_IKKE_OMSORG", "Far har ikke omsorg for barnet", 0),
    ...
    ES_STONAD_FLERE_GANGER("ES_STONAD_FLERE_GANGER", "Stønad er gitt for samme barn flere ganger", 0),
    //medlemskap

...
public class TilbakehoppTest {
    ...
    public void skal_kalle_transisjoner_på_steg_det_hoppes_over() {
        Assertions.assertThat(transisjonerVedTilbakehopp(fra(STEG_3, INNGANG), til(STEG_1))).containsOnly(StegTransisjon.hoppTilbakeOver(STEG_1), StegTransisjon.hoppTilbakeOver(STEG_2));
        Assertions.assertThat(transisjonerVedTilbakehopp(fra(STEG_3, INNGANG), til(STEG_2))).containsOnly(StegTransisjon.hoppTilbakeOver(STEG_2));
        Assertions.assertThat(transisjonerVedTilbakehopp(fra(STEG_2, UTGANG), til(STEG_2))).isEmpty();
    }

    @Test
    public void skal_ikke_ta_med_transisjon_på_steg_det_hoppes_til_for_overstyring() {
        Assertions.assertThat(transisjonerVedOverstyrTilbakehopp(fra(STEG_3, INNGANG), til(STEG_1))).containsOnly(StegTransisjon.hoppTilbakeOver(STEG_1), StegTransisjon.hoppTilbakeOver(STEG_2));
        Assertions.assertThat(transisjonerVedOverstyrTilbakehopp(fra(STEG_3, INNGANG), til(STEG_2))).containsOnly(StegTransisjon.hoppTilbakeOver(STEG_2));
        Assertions.assertThat(transisjonerVedOverstyrTilbakehopp(fra(STEG_2, UTGANG), til(STEG_2))).isEmpty();
    }
    ...
    private void assertAPReaktivertOgGjenåpnetVedTilbakehopp(StegPort fra, BehandlingStegType til, Aksjonspunkt ap) {
        ...
        Assertions.assertThat(aksjonspunkt.erAktivt()).isTrue();
        Assertions.assertThat(aksjonspunkt.getStatus()).isEqualTo(OPPRETTET);
    }

    private void assertAPInativtVedTilbakehopp(StegPort fra, BehandlingStegType til, Aksjonspunkt ap) {
        Aksjonspunkt aksjonspunkt = utførTilbakehoppReturnerAksjonspunkt(fra, til, ap);
        Assertions.assertThat(aksjonspunkt.erAktivt()).isFalse();
    }

package no.nav.foreldrepenger.tilbakekreving.batch.scheduler;
...
import no.nav.foreldrepenger.tilbakekreving.avstemming.batch.AvstemmingBatchTask;
...
public class BatchTaskScheduler implements AppServiceHandler {
    ...
    private void opprettTaskForType(Map.Entry<ProsessTaskType, ProsessTaskEntitet> entry) {
        ...
        if(AvstemmingBatchTask.BATCHNAVN.equals(type.getKode())){
            data.setProperty("dato", LocalDate.now().minusDays(1).format(DateTimeFormatter.ISO_DATE));
        }
